...
## 7.2 세 번째로 잡힌 버그 — 매핑을 메모리에서만 바꾸고 디스크에 안 씀

7.1절을 고친 뒤 다시 push하니 이번엔 실제로 Notion 페이지가 생성됐다(`created docs/Notion-Docs-Sync-Automation.md -> ...`). 그런데 바로 다음 줄에서 `nothing to commit or push failed: Command failed: git commit ...`가 떴다 — 매핑 파일이 안 바뀐 것처럼 커밋할 게 없다고 나온 것.

**원인**: `syncFile()`이 새 페이지 ID를 자바스크립트 객체 `map`에만 채워 넣었을 뿐(`map[file] = pageId`), 그 객체를 실제 `.notion-sync-map.json` 파일로 저장하는 `saveMap(map)` 호출을 `main()`에서 빼먹었다. 그래서 디스크상의 파일은 그대로였고, `git add`/`git commit`이 볼 변경사항 자체가 없었다.

**해결**: `main()`에서 `mapChanged`가 true면 커밋하기 전에 반드시 `saveMap(map)`을 호출하도록 한 줄 추가했다.
...
이번 건은 로그에 실패 원인이 그대로 찍혀서(파일 자체는 맞게 생성됐고, 커밋 단계만 실패) 원인 파악은 빨랐다. 다만 이 상태로 방치했으면 "페이지는 계속 새로 생기는데 매핑 파일에는 하나도 안 쌓이는" 상태가 됐을 것 — 다음 push마다 같은 파일을 또 새 페이지로 취급해 중복 생성했을 것이다. 이번엔 생성된 페이지 ID(`3e350b17-ecd1-81d0-a1cc-e06ba76d52c5`)를 매핑 파일에 수동으로 채워 넣어 복구했다.
...
| 회차 | 무엇을 했나 | 결과 |
|---|---|---|
| 1차 | 이 문서를 새로 커밋 | 워크플로우 `success`였지만 실제로는 아무 것도 안 함(7.1절) |
| 2차 | 7.1절 버그 고치고 재push | Notion 페이지는 생성됐지만 매핑 파일 커밋이 실패(7.2절) |
