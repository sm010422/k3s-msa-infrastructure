...
## 결론 요약 — 소프트웨어로 되는 것 vs 하드웨어가 필요한 것

| 하고 싶은 것 | 소프트웨어만으로 가능? |
|---|---|
| **애초에 안 자게 만들기** (예방) | ✅ 가능 — Amphetamine이 실제로 살아있는지 재확인, 더 견고하게 하려면 `caffeinate -disu`를 LaunchDaemon으로 등록. 이게 확실하면 "깨우기" 문제 자체가 안 생긴다 — **가장 먼저 해볼 것** |
| **잠들거나 죽었을 때 폰으로 즉시 알기** (감지) | ✅ 가능 — 아래 ①, 이미 구현·검증 완료 |
| **밖에서 신호 하나로 원격 wake-up** | ❌ 순수 소프트웨어로는 불가능 — 코드/설정 문제가 아니라 물리적 제약(WoL 매직 패킷은 로컬 브로드캐스트라 그걸 대신 쏴줄 두 번째 기기가 같은 집 네트워크에 있어야 함). **예외**: 지금 쓰는 공유기가 자체적으로 "Wake on WAN"을 지원하면 하드웨어 추가 없이 가능 — 공유기 모델부터 확인. 안 되면 라즈베리파이 같은 저가 상시 기기(~5만원대) 추가가 다음 단계 (아래 ②) |
